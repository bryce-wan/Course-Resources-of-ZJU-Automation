...
class protocol_packet_handler(object):
    ...
    def rxPacket(self, port):
        ...
        checksum = 0
        rx_length = 0
        # minimum length (HEADER0 HEADER1 ID LENGTH ERROR CHKSUM)
        wait_length = 6
    ...
    def ping(self, port, scs_id):
        ...
        if result == COMM_SUCCESS:
            data_read, result, error = self.readTxRx(
                port, scs_id, 3, 2)  # Address 3 : Model Number
            if result == COMM_SUCCESS:
    ...
    def read2ByteRx(self, port, scs_id):
        ...
        data_read = SCS_MAKEWORD(data[0], data[1]) if (
            result == COMM_SUCCESS) else 0
        return data_read, result, error
