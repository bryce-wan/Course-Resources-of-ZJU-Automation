...
u8 REF_TEMP[4] = {3, 0, 0, 0}; // modifiable
u8 dot = 0;
float diff;
float now_temp;
float ref_temp;
...
int main()
{
    ...
    while (1)
    {
        ...
        if (cnt > 200)
        {
            ...
            printf("now_temp: %f\r\n", now_temp);
            printf("ref_temp: %f\r\n", ref_temp);
            diff = now_temp > ref_temp ? now_temp - ref_temp : ref_temp - now_temp;
            printf("duty ratio: %f\r\n", (sqrt(diff) * 300 + 200) / 2000 * 100);
            printf("dot: %d\r\n", dot);
        }
        cnt++;
    }
...
void Drive(void)
{
    if (now_temp < ref_temp)
    {
        ...
        TIM_SetCompare2(TIM3, max(diff * 900 + 200, 2000));
    }
    else if (now_temp > ref_temp)
    {
        ...
        TIM_SetCompare1(TIM3, max(diff * 900 + 200, 2000));
