    ...
    def on_press(self, event):
        ...
        if (event.key == ' '):
            ...
            if self.planning_target is not None and self.planner is not None:
                print("do planning...")
                cal = planner(self.planning_obs, [self.x, self.y], self.planning_target, -
                              10, -10, 10, 10, 0.2, Playground.planning_obs_radius + DWAConfig.robot_radius + 0.4)
                px, py = cal.Process()
