    ...
    def solve(self):
        ...
        for i in range(len(self.end_lists)):
            ...
            for sgn1, sgn2 in sgn_lists:
                ...
                theta_4 = np.arcsin((oz * np.sin(theta_6) - nz * np.cos(theta_6)
                                     ) / np.cos(theta_5)) - theta_2 - theta_3

                th = np.array([theta_1, theta_2, theta_3,
                               theta_4, theta_5, theta_6])
                th = (
                    th - np.array([0, -np.pi/2, 0, np.pi/2, np.pi/2, 0])) * 180 / np.pi
...
if __name__ == "__main__":
    ...
    end_lists = [[-2.019, -0.058, -2.19, 0.117, 0.334, 0.499],
                 ...
                 [-0.35, 0.05, 0.3, 0.75 * np.pi, 0, -np.pi]]
